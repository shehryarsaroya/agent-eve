...
- **Last done (2026-07-24):** Six adversarial critics → SPEC v3.0 → three scoring panels → fixes integrated. Scored **7/10** on its own goals, **96/150** on the prior research's rubric, **ship-with-conditions** on engineering. See § SCORING PANEL for what was fixed and what was accepted-but-not-fixed.
...
| **Negotiation leaves our server** | Principals publish their own service endpoints; discovery, quoting, haggling and lying happen there; **only a countersigned `terms_hash` binds** | Three wins: the one-round-trip cap was a token bill, not a design choice, and it's gone · it restores the **noisy channel** the prior research required and all-public 140-char speech had nowhere to put · a principal with a published price list is a far more followable character than one that applies to slots. Not the forbidden side channel: the *conversation* is private, the *deal* is wholly public. |
