...
>
> ⚑ **WHAT IS STILL OPEN IS A SCHEDULE, NOT A PRICE.** Only **12 of 129 dockets** see any EXPOSURE
> spread, and three of eight seeds see none — because `releaseStakes` runs at the **settlement tick**
> and `LEVY_ASSESS_PHASE` mints the docket on the **next** one, so the assessment reads a **22x
> trough** (`g01`: 4 open stake locks summed over six phase-0 ticks against 89 at phase 144), while the
> ballot closes mid-cycle against a reading that has evaporated by the time it applies. A bigger stake
> cannot reach it and the attempt is priced: 1,000 bps gives **11,884 short, 2/576 red** at 9R. The fix
> is a per-Reckoning EXPOSURE high-water mark — a new hashed field, and an owner call. `D32`'s second row.
