...
describe('the book keeps a ruin, and keeps every number that was ever true about it', () => {
  it('drops out of every live accessor and out of none of the historical ones', () => {
    ...
    expect(book.sharesAt(SYS, YIELD_PER_TICK.COMMONS, at).size, 'a ruin takes no share').toBe(0);
...
describe('a campaign BREACH razes the defender\'s production, and a REBUFF never does', () => {
  ...
  function port(over: {
    ...
    return {
      ...
      // §16.12 #1's projection cap, pinned open so the razing arithmetic here is unaffected by it.
      // Its own behaviour belongs to `test/world/the-map-has-borders.spec.ts`, which reads a real
      // world rather than this fixture — the same split `pulse.spec.ts` makes.
      swayAt: () => Number.MAX_SAFE_INTEGER,
