...
function ventureRow(
  ...
  // Read ONCE and used twice — the figure and the direction it decides. `electiveCeilingOfRole` draws
  // proceeds at the p90 per role, so two calls would be two chances to disagree about one obligation
  // as well as twice the work on the heaviest row in the payload.
  const owedByMe = electiveOwedAtCeiling(runtime, venture, principal);
  return {
    id: venture.id,
    kind: venture.kind,
    state: venture.state,
    ...
    /**
     * ★ **WHAT THIS VENTURE CAN COST YOU TONIGHT** — summed over every role somebody else holds.
     *
     * ══════════════════════════════════════════════════════════════════════════
     * The creator's liability had no field on its own row. `my_elective` is zero for a creator that
     * holds no role, `my_elective_direction` was `null`, and the only carrier of the number was the
     * `elect` affordances — one per role, to be found and added up, and absent entirely outside a
     * wake (§12.4). Meanwhile `briefing.prompt` told the same principal it owed 4,800 on the same
     * payload, from a third arithmetic. *"Budget from the venture row — the obvious place — and you
     * default while believing you paid."*
     *
     * Same ceiling as the affordances, so this is their sum and not a fourth figure.
     * ══════════════════════════════════════════════════════════════════════════
     */
    my_elective_owed: owedByMe,
    /**
     ...
     *
     * ══════════════════════════════════════════════════════════════════════════
     * ★ **AND `OWED_BY_ME`, WHICH DID NOT EXIST.** The docstring used to end *"What YOU owe as a
     * creator is never here — it is the `elect` affordances"*, and that was a decision that read
     * fine and cost a real default: a creator with 4,800 riding on roles other principals filled got
     * `null` on the one field whose entire job is to say which way the money flows. Three values for
     * a receivable and none for a liability is a direction field that only points one way.
     *
     * A liability outranks a receivable, so it is checked FIRST — a creator that also holds a role of
     * its own is told what it owes rather than that its own share is self-paid, because only one of
     * those two facts can end in a public default. {@link my_elective_owed} carries the figure.
     * ══════════════════════════════════════════════════════════════════════════
     */
...
/**
 * ★ **THE SAME THING AT THE CEILING** — what this creator can actually be charged tonight.
 *
 * ══════════════════════════════════════════════════════════════════════════
 * {@link electiveOwed} above sums `role.terms.elective`, the **pinned price**, and feeds
 * `sign`'s `max_contingent_liability`. This one sums `electiveCeilingOf`, the **p90 bound**, which
 * is what the `elect` affordance publishes as `max_direct_loss` and what settlement can charge on a
 * share role that over-performs. On a HAUL at `elective_bps: 2500` the two differ by 60%.
 *
 * Both exist and both stay: a *contingent* liability on a venture that has not resolved is honestly
 * the pinned price, and a *direct* loss at settlement is honestly the ceiling. The defect a blind
 * player hit was that the venture row — the obvious place to budget from — published only the price,
 * with no field for the bound, so *"you default while believing you paid"*. This is the bound, and
 * it is deliberately the sum of the affordances rather than a fourth arithmetic for one obligation.
 * ══════════════════════════════════════════════════════════════════════════
 */
function electiveOwedAtCeiling(
  runtime: Runtime,
  venture: VentureRecord,
  principal: PrincipalId,
): number {
  ...
  for (const role of venture.roles) {
    if (role.filledByPrincipal === null) continue;
    if (role.filledByPrincipal === principal) continue;
    total += runtime.electiveCeilingOf(venture, role.index);
  }
  return total;
}
