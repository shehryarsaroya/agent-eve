...
| Phase 3 (risk market) | **~35%** | ⚑ **NO LONGER 0%** — `src/risk/` landed at `RULES_VERSION` 29: ~3,300 loc, eleven files, **49 tests**, **zero new verbs**. Built of §7–8's 27 MUST items: RSK1 · RSK3 · RSK4 · **RSK5 (the honour/default decision)** · RSK7 · **CAT1 (regional footprints, correlated loss)** · CAT2 · CAT4 · CAT5 · CAT6 · **RE1 (facultative reinsurance)** · RE6 · **SOL2 (phased clearing)** · SOL4 · GOV1's tier ladder · GOV4's bounded contagion. Measured: **a catastrophe propagates in 3 of 3 seeds when the primary is thin and 0 of 3 when it is deep** — contagion is a property of the mechanism *and* the balance sheet (SOL4), and a probe that had only run the deep case would have called the layer a tax. **A15 is 0/0/0 at N = 1/4/16.** The 35% cap is what is *not* built and is chosen rather than run out of: no pricing layer at all (RSK2/RSK6/CAT3/SOL1), one reinsurance shape of five (RE2–RE5), **no guaranty fund — GOV2 is a fifth currency faucet**, and **no heuristic cast branch**, so nothing in a world nobody steers has yet chosen to buy cover. §10.1's fourth sink — the FRONT — is the part that changes the rest of the game: `HAZARD` had no content for the project's entire life |
