...
- `PROP-O3` Exactly 11 top-level keys. A counting test, per §17 — the budget is enforced by arithmetic, not intentions. Likewise 15 axioms, ≤40 verbs, ≤8 venture kinds. **It was 10 until `risk` landed (2026-07-30):** the ceiling was raised by owner decision rather than a key traded away, because the alternative was an A9 violation — the spectator frame carried a FRONT the agent's own `observe` could not read. The test is unchanged in kind; only the number moved, and SPEC §12.1 carries the argument.
