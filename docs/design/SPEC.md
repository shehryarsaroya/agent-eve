...
| **`LODE_WEIGHT`** | **9..12** | a **1.33×** spread between the richest and poorest ground in a tier. The floor is set by the tightest tier's break-even, not by taste: at `8..13` the poorest MARCHES system clears its own burn by **192 units a Reckoning**, indistinguishable from zero. Tier totals are conserved exactly |
