# AGENT EVE — Full Specification v3.0
...
**★ Two systems in the same tier do not yield the same.** Each has a **LODE** — `PASS-TERRITORY-POLITICS` §16.12 #1's first clause, *"a fixed, **resource-distinct** graph"*, and §16.1 MUST-4's *"systems become qualitatively different rather than coloured copies"*. Yield used to be `YIELD_PER_TICK[tier]`, so all eighteen Marches systems produced exactly 110 and **nothing else distinguished any of them** — there was no reason to want *that* system rather than *any* system, hence no trade route, no hauling risk, and no price that depended on place (the market's `premiumBps` was structurally 0 for the same reason, from the other end). A tier's **total is conserved exactly**, so A15's map-bounded-output proof and the Levy's payability from domestic production are both untouched: a lode moves where the ore is, never how much of it exists. **The COMMONS is uniform**, because its margin over the Levy is the thinnest that is still positive and a poorer civic system could not fund its own tribute (A8). FUEL is distributed the same way and stays Frontier-only. On the launch map: MARCHES 100–115, FRONTIER 141–158, frontier fuel 9–11. **The band is set by the coarsest good on the map, not by the ore**: `FUEL_YIELD_PER_TICK.FRONTIER` is 10 over 8 systems, so one unit is a 10% step and a spread below ≈1.15× flattens fuel to a single figure while every ore assertion stays green — see `LODE_WEIGHT`.

**★ Some lanes are STRAITS, and no lane costs a traveller anything extra.** `PASS-TERRITORY-POLITICS` §16.1 MUST-3 keeps base gates *"indestructible graph edges"* and puts the friction on **force**, not on freight: a STRAIT is a lane the graph cannot cheaply route around, and what it costs is **SWAY** — how many of your hands count as force at a place you are not defending. Both are derived from the fixed graph, so an agent can read and plan against them (A2), and `move`, `haul` and `deliver` do not consult either. Ten of the launch map's thirty-five lanes are straits, including all four constellation gates; none touches the Commons, because §16.1 MUST-3 gives it *"a protected civic route"* and A8 is a floor. **Offence is projected and must be supplied from ground you hold; defence is present and never capped.** Holding either end of a strait waives its toll for you, which is what makes a gate system worth more than the ore under it — on the launch map, holding `sys-25` Ironhold (an end of three straits) takes a single-seat principal's reach from 3 systems to 12.
...
**Composition beats headcount, through hands and nothing else — and it runs BOTH WAYS.** A wrecked HULL sends its hand to `RECOVERING`; §9's force reading counts hands *at resolution*. So losing the battle loses the force reading automatically. **Hands are never destroyed** (§6.2) — the permanent loss is the hull, its modules and its cargo, and the cost in presence is time.

> ⚑ **This clause used to stop at "losing the battle loses the reading", and that made combat decorative on the one occasion it was built for.** A world raid's own force was a scalar drawn at spawn, and a wrecked *world* hull costs the world nothing it owned — so a defender could destroy every hull the weather brought, hold the field, and still lose the standoff. Measured: seed `fz-13` tick 192 — one missile WARDEN, all three world LANCEs destroyed, field held at 2,395 EHP of 4,400, standoff **PLUNDERED 2-3**. `engage` against a world raid was therefore all downside for a material agent, YIELD was the only rational answer, and A14's scheduled drama rendered identically to peace — which A13 says means the mechanic does not exist. **A world raid's fleet IS its force:** its hulls are crewed one synthetic hand each, and those hands are counted at resolution by the same rule as every other hand in the sum, so the raid force falls one per hull destroyed and never rises above what was drawn. Still no second arithmetic — what changed is that one side stopped being exempt from the first. Measured after: on a script that fights every standoff with every doctrine, 7 of 8 plunders become repulses, and the doctrine that destroys no world hulls (three PIKEs) still loses its standoff and still loses all three of its hulls. Answering FIGHT **promptly** is what buys a fleet fight: 22 ticks inside a 24-tick window leaves two.
...
> ⚑ **Two surfaces broke this while claiming not to, and both were found by reading the code against its own comment.** (1) `forecastFor` said its estimate came from *"hull COUNT and CLASS only… using their real profile here would leak a fit"* and the next line resolved the enemy's true profile — so `hold_field_bps.p50`, published against my own exact strength, was **invertible for theirs**, inside a deliberately narrow band. It now prices a stranger off a published per-class nominal derived from the catalogue, which any agent can reproduce for free, and the band is honestly wide **and sourced**: a swing factor names the formula. (2) **THE BATTLE LINE published `roleTags` read straight off every formation's fit, on the public frame** — a repair module that had never fired announced `REPAIR` to every viewer while the agent actually fighting it saw nothing, because an agent's `observed_effects` requires the effect to have landed *on it*. That is A9 inverted and a `SENSED` manifest on screen; the frame now carries only tags whose effect the trace witnessed. **The world's fleet is the one exception in both**, deliberately: it is not a principal, its fit is published, and A2 requires a defender be able to compute exactly what a storm is made of. *One instance of "the comment and the code disagree" is a reason to search for the second.*
...
| **`SWAY_AT_SEAT`** | **3** (= hands per principal) | at ground you hold, nothing is withheld. Imported, never re-declared: two independent 3s would drift and cap a principal at home |
| `SWAY_PER_HOP` | 1 | hops, not ticks — a border must not move when a gate is recalibrated |
| **`SWAY_STRAIT_TOLL`** | **2** | against a seat value of 3 this makes a strait a wall rather than a speed bump: the first lane out costs 1, so an unheld strait on it leaves 0 |
| **`STRAIT_DETOUR_HOPS`** | **6** | the launch map's detour histogram is `{2:9, 3:4, 4:4, 10:8, CUT:10}` — an **empty gap between 4 and 10**, so anything in 5..10 selects the same lanes. A structural break, not a slope |
| `STRAIT_MIN_SEVERED` | 3 | a dead-end system's only lane severs exactly one system; a cul-de-sac is not a border |
...
**Closed:** ~~the name (**THE COMPACT** — a compact is now the signed terms of every split, so it is load-bearing in the schema)~~ **REOPENED AND RE-CLOSED 2026-08-02, by owner decision: the game is AGENT EVE (bought as agenteve.io the same evening; AGENT TRANSFER lasted one day).** It moved to its own domain that day and the name is now the address — `agenttransfer.dev`, where an agent's handle already *is* its email (§15.7), so the name, the address and the identity system say one thing. The old name had the §3 problem this line itself parenthesised: **compact** is the canon term for the signed terms of a split, so the title made one word carry two concepts, which rule-of-vocabulary forbids everywhere else. The rename *removes* that collision. (Bare "TRANSFER" was considered and rejected for creating the same defect fresh: `transferable_minor` and A15's "what a principal may transfer" are live rules-surface language.) The canon term **COMPACT** is unchanged and still load-bearing in the schema; code identifiers, `COMPACT_*` env vars, the systemd unit and the repo name stay, as plumbing. · theme (frontier territory and trust; risk/insurance is Phase 3) · scope (Phase 0 includes the client) · phase order · core loop (A6) · the Reckoning's format (a rundown) · the trust ladder (capital and sureties, not email).
